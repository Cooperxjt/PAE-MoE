...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _unload_and_optionally_merge(self, merge=True):
        # 检查模型是否以8位或4位模式加载，这种模式下无法合并LORA层
        if getattr(self.model, "is_loaded_in_8bit", False) or getattr(self.model, "is_loaded_in_4bit", False):
            raise ValueError("Cannot merge LORA layers when the model is loaded in 8-bit mode")

        # 获取所有不包含"lora"的非空模块名
        key_list = [key for key, _ in self.model.named_modules() if "lora" not in key and key != '']

        for key in key_list:
            try:
                # 获取模块的父模块、目标模块、名称和层信息
            ...
            if isinstance(target, LoraLayer):
                ...
                # 如果启用合并，将LORA权重合并到基础层中
                if merge:
                    target.merge()
                # 用新的基础模块替换LORA层
                self._replace_module(parent, target_name, new_module, target)

            # 保存属于`modules_to_save`的任何额外可训练模块
