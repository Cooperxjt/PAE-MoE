...
def _get_submodules(model, key):
    parts = key.split(".")  # 只拆分一次，避免重复计算
    # 校验1：路径为空，直接抛出明确错误（或跳过）
    if not parts or parts == ['']:
        raise ValueError(f"Invalid empty module key: {key}")
    # 校验2：获取父模块（处理根模块情况，虽然已过滤，但做双重保障）
    if len(parts) == 1:
        parent = model  # 只有1层路径，父模块是模型自身
        target_name = parts[0]
    else:
        parent = model.get_submodule(".".join(parts[:-1]))
        target_name = parts[-1]
    # 修复核心：优雅提取层索引（model.layers.xx），而非硬编码取索引2
    layer = None
    if "layers" in parts:
        # 找到"layers"的索引，下一个元素就是层索引（如['model','layers','29',...] → 29）
        layers_idx = parts.index("layers")
        if layers_idx + 1 < len(parts):
            layer = parts[layers_idx + 1]
    # 获取当前模块实例
    target = model.get_submodule(key)
    # 返回：父模块、当前模块、模块名、层索引（无则为None）
