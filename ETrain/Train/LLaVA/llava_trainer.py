...
class LLaVATrainer(Trainer):
    ...
    def create_optimizer(self):
        """
        Setup the optimizer with a dedicated LR for shared-expert LoRA params.
        """
        ...
        if self.optimizer is None:
            # ---- 1) 计算 decay 参数名（保持你原逻辑） ----
            decay_parameters = get_parameter_names(opt_model, ALL_LAYERNORM_LAYERS)
            decay_parameters = [name for name in decay_parameters if "bias" not in name]

            # ---- 2) 识别 shared LoRA 参数（按名字匹配 expert 0）----
            # 你目前 LoRA 的共享专家一般是 expert_id=0，典型名字包含：
            #   ...lora_A.<adapter>.loraA.0.mlp.weight
            #   ...lora_B.<adapter>.loraB.0.mlp.weight
            # 如果你的命名不是这样，你只需要改下面这两个 pattern。
            def is_shared_lora_param(name: str) -> bool:
                return (
                    (".lora_A." in name and ".loraA.0." in name)
                    or (".lora_B." in name and ".loraB.0." in name)
                )

            # ---- 3) 收集参数，确保不重复进组 ----
            shared_decay, shared_nodecay = [], []
            other_decay, other_nodecay = [], []

            # 用来去重（有的参数可能被不同名字引用，保险起见）
            seen = set()
            ...
            for n, p in opt_model.named_parameters():
                ...
                is_shared = is_shared_lora_param(n)

                if is_shared:
                    if is_decay:
                        shared_decay.append(p)
                    else:
                        shared_nodecay.append(p)
                else:
                    if is_decay:
            ...
            # ---- 4) 组装 param groups，给 shared 单独 lr ----
            optimizer_cls, optimizer_kwargs = Trainer.get_optimizer_cls_and_kwargs(self.args)

            # base lr 一般在 optimizer_kwargs 里（来自 TrainingArguments.learning_rate）
            base_lr = optimizer_kwargs.get("lr", self.args.learning_rate)

            # 你可以先写死 0.1，后续再加到 args 里作为超参
            shared_lr_mult = getattr(self.args, "shared_lr_mult", 0.01)
            print("shared_lr_mult", shared_lr_mult)
            shared_lr = base_lr * float(shared_lr_mult)

            optimizer_grouped_parameters = []

            # shared groups（lr 更小）
            if len(shared_decay) > 0:
                optimizer_grouped_parameters.append({
                    "params": shared_decay,
                    "weight_decay": self.args.weight_decay,
                    "lr": shared_lr,
                })
            if len(shared_nodecay) > 0:
                optimizer_grouped_parameters.append({
                    "params": shared_nodecay,
                    "weight_decay": 0.0,
                    "lr": shared_lr,
                })

            # other groups（默认 lr）
            if len(other_decay) > 0:
                optimizer_grouped_parameters.append({
                    "params": other_decay,
                    "weight_decay": self.args.weight_decay,
                    "lr": base_lr,
                })
            if len(other_nodecay) > 0:
                optimizer_grouped_parameters.append({
                    "params": other_nodecay,
                    "weight_decay": 0.0,
                    "lr": base_lr,
                })

            # ---- 5) 创建 optimizer（保持你原逻辑）----
            if self.sharded_ddp == ShardedDDPOption.SIMPLE:
                self.optimizer = OSS(
                ...
                # bitsandbytes embedding 特例（保持你原逻辑）
            ...
            if self.is_world_process_zero():
                group_info = [(i, g.get("lr", base_lr), g.get("weight_decay", None), len(g["params"]))
                            for i, g in enumerate(self.optimizer.param_groups)]
                print("[Optimizer groups] idx, lr, wd, n_params =", group_info)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        for mod in core.modules():
            v = getattr(mod, "_gate_reg_loss", None)
            if v is not None:
                gate_reg = v if gate_reg is None else (gate_reg + v)

        if gate_reg is not None:
            loss = loss + 0.1 * gate_reg

        # Router loss（来自 MultiExpertProjector.forward 中计算）
        router_loss = getattr(core.mm_projector, '_router_loss', None)
    ...
    def save_trained_model(self, training_args):
        if training_args.lora_enable:
            ...
            if training_args.local_rank == 0 or training_args.local_rank == -1:
                ...
                # 保存 Router
                try:
                    core = self.model.base_model.model.model
                    rm = getattr(core.mm_projector, 'router_manager', None)
                    if rm is not None:
                        router_dir = os.path.join(training_args.output_dir, 'router')
                        rm.save(router_dir)
                except Exception as e:
                    print(f'[Router] Save failed: {e}')
        else:
    ...
    def before_train(self):
        ...
        list_data_dict_new = []
        for index in rand_dom:
            list_data_dict_new.append(train_dataset.list_data_dict[index])
        train_dataset.list_data_dict = list_data_dict_new
        ...
        logits = {}
        # for idx, inputs in enumerate(train_dataloader.dataset.list_data_dict):
        #     logits.append({inputs['question_id']:{}})
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    ...
                    for idx, question_id in enumerate(inputs['question_ids']):
                        logits[question_id] = outputs['logits'][idx].clone().cpu()
                        # for index, logit in enumerate(logits):
                        #     if question_id in logit.keys():
                        #         logits_index.append(index)
                        #         logit[question_id] = outputs['logits'][idx].clone().cpu()
        ...
        # logits_results = []
        # for index in logits_index:
        #     logits_results.append(logits[index])
        # dist.all_gather_object(outputs, logits_results)

        torch.save(logits, os.path.join(self.args.output_dir, 'logts_{}.bin'.format(dist.get_rank())))

        dist.barrier()

        outputs = [torch.load(os.path.join(self.args.output_dir, 'logts_{}.bin'.format(i)), map_location='cpu') for i in range(dist.get_world_size())]
        final_logits = {}
        for rank_output in outputs:
            for key, value in rank_output.items():
                if key not in final_logits.keys():
                    final_logits[key] = value

        # final_logits = {}
        # for rank_output in outputs:
        #     for single_sample in rank_output:
        #         for key, value in single_sample.items():
        #             if key not in final_logits.keys():
        #                 final_logits[key] = value

        # model.module.base_model.model.previous_logits = final_logits
        print('Finished training previous logits...')
        return final_logits
                    ...
                    for n, p in model.module.base_model.model.named_parameters():
                        if p.requires_grad:
                            ...
                            if hp_grad is not None:
                                fisher[n].data += hp_grad.data.cpu() ** 2
